// Frontend wrapper for LocationEngine logic (mirrors backend implementation)
import { BusStatus, LocationSource, TripLocationResponse, TripStopInfo, UpcomingStop } from '../../../shared/types';
...
export class LocationEngine {
  static getCurrentLocation(trip: any, delayMinutes: number = 0): TripLocationResponse {
    ...
    return {
      ...
      currentLocation,
      currentSegment,
      passedStops: passedStops.map(s => LocationEngine.mapStop(s)),
      upcomingStops: upcomingStops.map(s => {
        const t = s.timeMins;
        return {
          ...LocationEngine.mapStop(s),
